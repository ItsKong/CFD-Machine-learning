...
BASE_CONFIG = "turb_SA_RAE2822.cfg"      # your base config (do not edit this file)
MESH_FILE   = "mesh_RAE2822_turb.su2"    # must be in the same folder as this script
SU2_BIN     = "D:/SU2/bin/SU2_CFD"                  # or full path e.g. "/usr/local/bin/SU2_CFD"
OUTPUT_DIR  = Path("../data/batch_su2_cases")          # all case folders go here
...
def prepare_case(aoa: float) -> tuple:
    ...
    cfg = Path(BASE_CONFIG).read_text()
    ...
    mesh_link = case_dir / MESH_FILE
    if not mesh_link.exists():
        mesh_src = Path(MESH_FILE).resolve()
        try:
...
def run_case(args) -> dict:
    ...
    tag      = f"{aoa:.2f}".replace("-", "neg").replace(".", "p")
    csv_candidates = list(case_dir.glob(f"surface_flow.csv"))
    # Convert vtu file
